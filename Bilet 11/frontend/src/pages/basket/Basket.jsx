        ...
        <tbody>
          {
            basket && basket.map((item) => (
              <tr key={item._id} style={{ verticalAlign: "middle" }}>
                ...
                <td>${item.price}</td>
                <td>
                  <button className='btn-basket' disabled={item.count==1} onClick={() =>dispatch(minusBtn(item._id))}>-</button>
                  {item.count}
                  <button className='btn-basket' onClick={() => dispatch(plusBtn(item._id))}>+</button>
                </td>
